...
function createVideoHistory({ db, assetsDir, maxBytes, maxEntries = 9 }) {
  ...
  async function list({ scope, roleName }) {
    ...
    for (const record of sorted) {
      ...
      try {
        ...
        // Missing old assets should not make the current chat unusable.
      }
  ...
  async function load({ scope, roleName, referenceId }) {
    ...
    try {
      const video = await fs.promises.readFile(record.localPath);
