...
function getPhysicalStateAtScheduleIndex(
  ...
  let result = clonePhysicalState(initialState);
  ...
  for (let cursor = 0; cursor <= lastIndex; cursor += 1) {
    result = mergePhysicalState(result, entries[cursor]);
    for (const event of eventsByIndex.get(cursor) || []) {
      result = mergePhysicalState(result, event.updates);
    }
  }
  return result;
}
...
function createRoleScheduleManager({
  ...
  async function syncRuntimeState({
    ...
    if (existing?.dateKey === schedule.dateKey && existing.entryKey === entryKey) {
      ...
      if (physicalStateNeedsUpgrade) {
        const fields = {
          physicalStateVersion: PHYSICAL_STATE_VERSION,
          physicalState: mergedPhysicalState,
        };
        const physicalStateChanges = buildPhysicalStateChanges(
          existingPhysicalState,
          mergedPhysicalState,
        );
        if (Object.keys(physicalStateChanges).length > 0) {
          fields.physicalStateChanges = physicalStateChanges;
        }
        for (const field of ["outfit", "carriedItems"]) {
          if (hasOwn(mergedPhysicalState, field)) {
            fields[field] = mergedPhysicalState[field];
          }
        }
  ...
  async function ensureDailySchedule(roleOrName, at = now()) {
    ...
    const dailySeed = getDailyScheduleSeed(roleName, dateKey);
  ...
  function buildRuntimeContextFromState(state) {
    ...
    const continuityLines = [
      ...
      !configuredVideoLocationGuard
        ...
        : "在本轮回复、图片和视频中保持当前地点、环境、活动、穿着、随身物品、手持物品、内部装置、身体状态和四肢状态连续；只有日程明确记录变化时才切换。",
      "不要向用户解释状态机或后台记录。",
    ].filter(Boolean);
  ...
  async function maybeSendProactive(session, at = now()) {
    ...
    if (proactiveLocks.has(proactiveKey)) {
      return { sent: false, reason: "in-flight", state };
    }
    ...
    const promise = (async () => {
      ...
      // A schedule entry may last for hours. It is still one event: once it
      // has been claimed or sent, never send the same entry again after the
      // global cooldown expires.
      if (existing && (
        existing.sentAt ||
        existing.attemptedAt ||
        ["sending", "sent", "failed"].includes(existing.status)
      )) {
        return { sent: false, reason: "entry-already-handled", state };
      }

      const latestSentAt = previousRecords
        .map((record) => Date.parse(record.sentAt || record.completedAt || ""))
      ...
      try {
        ...
        const alreadyClaimed = await db.findOneAsync(scheduleQuery);
        if (alreadyClaimed) {
          return { sent: false, reason: "entry-already-handled", state };
        }
        throw error;
