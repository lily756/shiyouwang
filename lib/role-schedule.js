...
const LONG_TERM_PLAN_RECORD_TYPE = "role-long-term-plan";
const WEEKLY_PLAN_RECORD_TYPE = "role-weekly-plan";
const DAY_PLAN_RECORD_TYPE = "role-next-day-plan";
...
const MAX_SYNTHETIC_FREE_TAIL_MINUTES = 3 * 60;
const MAX_SYNTHETIC_FREE_TOTAL_MINUTES = 4 * 60;
const DAILY_SEED_NAMESPACE = "role-daily-plan";
const DAILY_SEED_VERSION = 1;
const LONG_TERM_PLAN_SEED_NAMESPACE = "role-long-term-plan";
const WEEKLY_PLAN_SEED_NAMESPACE = "role-weekly-plan";
const DAY_PLAN_SEED_NAMESPACE = "role-next-day-plan";
...
function getWeekDateKeys(weekKey) {
  const start = normalizeDateKey(weekKey);
  if (!start) {
    return [];
  }
...
function getWeekdayIndex(dateKey) {
  const weekKey = getWeekStartDateKey(dateKey);
  const dates = getWeekDateKeys(weekKey);
  return Math.max(0, dates.indexOf(normalizeDateKey(dateKey)));
}

function getPlanningSeed(namespace, roleName, key = "") {
  return hashSeedText([
    namespace,
    PLANNING_VERSION,
    normalizeRoleNameKey(roleName),
    String(key || ""),
  ].join(":"));
}

function normalizePlanningReference(value, maximum = 96) {
...
function makePlanningId(prefix, value, fallback) {
  const normalized = normalizePlanningReference(value, 72);
  if (normalized) {
    return normalized === prefix || normalized.startsWith(`${prefix}-`)
      ? normalized
      : `${prefix}-${normalized}`;
  }
  return `${prefix}-${hashSeedText(String(fallback || prefix)).toString(36)}`;
}

function normalizePlanningPriority(value, fallback = 2) {
  return clampInteger(value, fallback, 1, 3);
}

function normalizePlanningStatus(value, fallback = "active") {
  const status = String(value || "").trim().toLocaleLowerCase();
  return ["active", "paused", "completed"].includes(status) ? status : fallback;
}

function normalizePlanningTextList(value, maximumItems = 8, maximumLength = 180) {
...
function normalizePlanGoal(raw, index = 0) {
  const source = typeof raw === "string" ? { title: raw } : raw;
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    return null;
  }
  const title = normalizeText(source.title ?? source.name ?? source.goal, "", 180);
  if (!title) {
    return null;
  }
  const targetDate = normalizeDateKey(source.targetDate ?? source.target_date);
  return {
    id: makePlanningId("goal", source.id, `${index}:${title}`),
    title,
    area: normalizeText(source.area ?? source.category, "生活", 80),
    priority: normalizePlanningPriority(source.priority, index === 0 ? 1 : 2),
    status: normalizePlanningStatus(source.status),
    progress: clampInteger(source.progress, 0, 0, 100),
    ...(targetDate ? { targetDate } : {}),
    ...(normalizeText(source.note ?? source.notes, "", 240)
      ? { note: normalizeText(source.note ?? source.notes, "", 240) }
      : {}),
  };
}

function normalizePlanGoals(value) {
  const goals = Array.isArray(value) ? value : [];
  const usedIds = new Set();
  const result = [];
  for (const [index, raw] of goals.entries()) {
    const goal = normalizePlanGoal(raw, index);
    if (!goal || usedIds.has(goal.id)) {
      continue;
    }
    usedIds.add(goal.id);
    result.push(goal);
...
function normalizePlanCommitment(raw, index = 0, { weekKey = "", requireTime = false } = {}) {
  const source = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
  const dateKey = normalizeDateKey(source.dateKey ?? source.date ?? source.day);
  const startMinute = parseTime(source.startMinute ?? source.start ?? source.from);
  const endMinute = parseTime(source.endMinute ?? source.end ?? source.to, { allowEndOfDay: true });
  if (!dateKey || (weekKey && getWeekStartDateKey(dateKey) !== weekKey) ||
      (requireTime && (startMinute === null || endMinute === null || endMinute <= startMinute))) {
    return null;
  }
  const activity = normalizeText(source.activity ?? source.title ?? source.name, "", 180);
  if (!activity) {
    return null;
  }
  const location = normalizeLocation(source.location ?? source.place) || "家";
  const environment = normalizeText(source.environment ?? source.setting, location, 160);
  const goalId = normalizePlanningReference(source.goalId ?? source.goal_id, 96);
  return {
    id: makePlanningId("commitment", source.id, `${dateKey}:${index}:${activity}`),
    dateKey,
    ...(startMinute !== null ? { startMinute } : {}),
    ...(endMinute !== null && endMinute > startMinute ? { endMinute } : {}),
    kind: normalizeKind(source.kind, activity),
    activity,
    location,
    environment,
    priority: normalizePlanningPriority(source.priority, 1),
    ...(goalId ? { goalId } : {}),
  };
}

function normalizePlanCommitments(value, options = {}) {
  const values = Array.isArray(value) ? value : [];
  const usedIds = new Set();
  const result = [];
  for (const [index, raw] of values.entries()) {
    const commitment = normalizePlanCommitment(raw, index, options);
    if (!commitment || usedIds.has(commitment.id)) {
      continue;
    }
  ...
  return result.sort((left, right) => (
    ...
    Number(left.startMinute ?? 1441) - Number(right.startMinute ?? 1441)
  ));
}

function normalizeDayPlanEntries(value) {
  const values = Array.isArray(value) ? value : [];
  const candidates = [];
  for (const [index, raw] of values.entries()) {
    ...
    const startMinute = parseTime(raw.startMinute ?? raw.start ?? raw.from);
    const endMinute = parseTime(raw.endMinute ?? raw.end ?? raw.to, { allowEndOfDay: true });
    const activity = normalizeText(raw.activity ?? raw.title ?? raw.name, "", 180);
    if (startMinute === null || endMinute === null || endMinute <= startMinute || !activity) {
      continue;
    }
    const location = normalizeLocation(raw.location ?? raw.place) || "家";
    const goalId = normalizePlanningReference(raw.goalId ?? raw.goal_id, 96);
    const commitmentId = normalizePlanningReference(raw.commitmentId ?? raw.commitment_id, 96);
    candidates.push({
      id: makePlanningId("day", raw.id, `${index}:${startMinute}:${activity}`),
      startMinute,
      endMinute,
      kind: normalizeKind(raw.kind, activity),
      activity,
      location,
      environment: normalizeText(raw.environment ?? raw.setting, location, 160),
      mood: normalizeText(raw.mood, "平常状态", 80),
      priority: normalizePlanningPriority(raw.priority, 2),
      ...(goalId ? { goalId } : {}),
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.endMinute <= startMinute) {
      continue;
    }
    result.push({ ...candidate, startMinute });
...
function isGenericFreeEntry(entry) {
  return !isTransitionEntry(entry) &&
    entry?.kind === "rest" &&
    String(entry?.activity || "").trim() === "自由安排 / 休息" &&
    String(entry?.environment || "").trim() === "按当天情况自然变化";
}

function normalizeGenericFreeEntry(entry) {
  if (!isGenericFreeEntry(entry)) {
    return entry;
  }
  const normalized = {
    ...entry,
  ...
  // These fields are meaningful only for a real move. Leaving model-invented
  // travel hints on a filler entry prevents equivalent fragments from being
  // compacted and can create a false movement chain later.
  delete normalized.destination;
  delete normalized.preparationMinutes;
  delete normalized.travelMinutes;
  delete normalized.movement;
  delete normalized.indoorMovement;
  return normalized;
}

function getScheduleEntryMetadata(entry) {
  const { startMinute: _startMinute, endMinute: _endMinute, ...metadata } = entry || {};
  return JSON.stringify(metadata);
}

function mergeAdjacentGenericFreeEntries(entries) {
  const merged = [];
  for (const rawEntry of Array.isArray(entries) ? entries : []) {
    const entry = normalizeGenericFreeEntry(rawEntry);
    const previous = merged.at(-1);
    if (
      previous &&
      isGenericFreeEntry(previous) &&
      isGenericFreeEntry(entry) &&
      Number(previous.endMinute) === Number(entry.startMinute) &&
      normalizeLocationKey(previous.location) === normalizeLocationKey(entry.location) &&
      getScheduleEntryMetadata(previous) === getScheduleEntryMetadata(entry)
    ) {
...
function hasAcceptableSyntheticFreeTime(entries) {
  const values = Array.isArray(entries) ? entries : [];
  const fillerEntries = values.filter(isGenericFreeEntry);
  const totalMinutes = fillerEntries.reduce(
    (total, entry) => total + Math.max(0, Number(entry.endMinute) - Number(entry.startMinute)),
    0,
  );
  ...
  for (let index = values.length - 1; index >= 0; index -= 1) {
    ...
    trailingMinutes += Math.max(0, Number(entry.endMinute) - Number(entry.startMinute));
  }
  return totalMinutes <= MAX_SYNTHETIC_FREE_TOTAL_MINUTES &&
    trailingMinutes <= MAX_SYNTHETIC_FREE_TAIL_MINUTES;
}

function hasUsableDailySchedule(entries) {
  return Array.isArray(entries) &&
    entries.length > 0 &&
    hasLocationCoverage(entries) &&
    hasAcceptableSyntheticFreeTime(entries);
}
...
function fillScheduleGaps(entries) {
  ...
  return mergeAdjacentGenericFreeEntries(result);
}
...
function normalizeScheduleEntries(payload) {
  const rawEntries = Array.isArray(payload)
    ...
    : Array.isArray(payload?.entries)
      ? payload.entries
      : [];
  if (rawEntries.length > MAX_SCHEDULE_ENTRIES * 2) {
    return [];
  }
  const candidates = [];
  ...
  for (const raw of rawEntries) {
    ...
    const goalId = normalizePlanningReference(raw.goalId ?? raw.goal_id, 96);
    const commitmentId = normalizePlanningReference(
      raw.commitmentId ?? raw.commitment_id,
      96,
    );
  ...
  candidates.sort((left, right) => left.startMinute - right.startMinute || left.endMinute - right.endMinute);
  const continuitySafeCandidates = mergeAdjacentGenericFreeEntries(
    enforcePhysicalStateContinuity(candidates),
  );
  // Do not silently truncate a model response that expanded one activity into
  // dozens of tiny rows. Returning no schedule lets the deterministic seeded
  // planner take over instead of filling the remainder of the day with a
  // misleading location-bound rest entry.
  if (continuitySafeCandidates.length > MAX_SCHEDULE_ENTRIES) {
    return [];
  }
  const entries = [];
  let cursor = 0;
  ...
  const compactEntries = mergeAdjacentGenericFreeEntries(entries);
  const canonicalEntries = hasValidMovementTransitions(compactEntries)
    ? compactEntries
    : fillScheduleGaps(compactEntries.filter((entry) => !isTransitionEntry(entry)));
  return mergeAdjacentGenericFreeEntries(insertMovementTransitions(canonicalEntries));
}

function getRolePlanningProfile(role = {}, random = Math.random) {
  const source = [role?.name, role?.description, role?.systemPrompt]
    .filter(Boolean)
    .join(" ");
  const isStudent = /(学生|学校|上课|课程|考试|学习)/u.test(source);
  const isCreative = /(作家|画家|音乐|创作|设计|艺术|摄影|写作|程序员|开发)/u.test(source);
  const isRemote = /(远程|居家|宅|在家工作|自由职业)/u.test(source);
  const home = "家";
  ...
  return {
    ...
    mainLocation,
    mainEnvironment,
    mainKind: isStudent ? "study" : isCreative ? "creative" : "work",
  };
}

function buildSeededLongTermPlan({ role = {} } = {}) {
  const random = createSeededRandom(
    getPlanningSeed(LONG_TERM_PLAN_SEED_NAMESPACE, role?.name),
  );
  const profile = getRolePlanningProfile(role, random);
  const primary = profile.isStudent
    ? "稳步完成本阶段学习与课程任务"
    : profile.isCreative
      ? "持续推进一项能留下成果的个人创作"
      : "稳步推进手头最重要的一项工作或个人项目";
  const primaryArea = profile.isStudent ? "学习" : profile.isCreative ? "创作" : "工作";
  return {
    summary: `${role?.name || "角色"}会在规律生活中逐步推进重要事项，同时保留休息、运动和关系联结。`,
    goals: normalizePlanGoals([
      {
        id: "primary",
        title: primary,
        area: primaryArea,
        priority: 1,
        status: "active",
        progress: seededInteger(random, 5, 22),
        note: "以连续、可完成的小阶段推进，不用一天做完。",
      },
      {
        id: "wellbeing",
        title: "保持稳定作息并完成规律的轻度运动",
        area: "身心状态",
        priority: 2,
        status: "active",
        progress: seededInteger(random, 8, 30),
      },
      {
        id: "connection",
        title: "给重要关系和个人兴趣留出可持续的时间",
        area: "关系与生活",
        priority: 3,
        status: "active",
        progress: seededInteger(random, 5, 25),
      },
    ]),
    routines: profile.isStudent
      ? ["每天留出一段专注学习时间", "尽量在夜间获得足够睡眠", "每周安排数次散步或轻运动"]
      : profile.isCreative
        ? ["每天推进一小段创作", "给灵感整理和恢复留出空白", "每周安排数次散步或轻运动"]
        : ["每天推进一项主要事务", "晚间为次日做简短准备", "每周安排数次散步或轻运动"],
    constraints: [
      "睡眠、用餐和必要移动优先于临时加塞的普通任务。",
      "计划变更时优先重排尚未发生的部分，不改写已经发生的状态。",
    ],
  };
}

function buildSeededWeeklyPlan({ role = {}, weekKey = "", longTermPlan = null } = {}) {
  const normalizedWeekKey = getWeekStartDateKey(weekKey);
  const dates = getWeekDateKeys(normalizedWeekKey);
  const random = createSeededRandom(
    getPlanningSeed(WEEKLY_PLAN_SEED_NAMESPACE, role?.name, normalizedWeekKey),
  );
  const profile = getRolePlanningProfile(role, random);
  const goals = normalizePlanGoals(longTermPlan?.goals);
  const primary = goals.find((goal) => goal.status === "active" && goal.priority === 1) || goals[0];
  const wellbeing = goals.find((goal) => goal.id === "goal-wellbeing") || goals[1] || primary;
  const connection = goals.find((goal) => goal.id === "goal-connection") || goals[2] || wellbeing || primary;
  const goalForDay = [primary, primary, wellbeing, primary, primary, connection, wellbeing];
  const dailyFocus = dates.map((dateKey, index) => {
    const goal = goalForDay[index] || primary;
    const isWeekend = index >= 5;
    const title = goal?.title
      ? isWeekend
        ? `以更轻松的节奏照顾「${goal.title}」`
        : `推进「${goal.title}」的一个可完成阶段`
      : "完成当天最重要的一件事";
    return {
      dateKey,
      ...(goal?.id ? { goalId: goal.id } : {}),
      title,
      mode: isWeekend ? "light" : "focus",
    };
  });
  const reviewDayIndex = seededInteger(random, 3, 5);
  const reviewDateKey = dates[reviewDayIndex] || normalizedWeekKey;
  const reviewGoalId = primary?.id || "";
  return {
    weekKey: normalizedWeekKey,
    summary: profile.isStudent
      ? "本周以学习推进、休息恢复和整理节奏为主。"
      : profile.isCreative
        ? "本周以创作推进、灵感整理和恢复节奏为主。"
        : "本周以主要事项推进、生活维护和恢复节奏为主。",
    focusGoalIds: goals
      ...
      .map((goal) => goal.id),
    dailyFocus,
    commitments: normalizePlanCommitments([
      {
        id: `weekly-review-${normalizedWeekKey}`,
        dateKey: reviewDateKey,
        startMinute: 19 * 60 + 30,
        endMinute: 20 * 60 + 20,
        kind: "routine",
        activity: "整理本周进度，并为接下来的安排做准备",
        location: "家",
        environment: "书桌或安静的工作角",
        priority: 2,
        ...(reviewGoalId ? { goalId: reviewGoalId } : {}),
      },
    ], { weekKey: normalizedWeekKey, requireTime: true }),
    outcomes: [],
  };
}

function overlayDayPlanEntry(entries, replacement) {
  const next = [];
  for (const entry of entries) {
    if (entry.endMinute <= replacement.startMinute || entry.startMinute >= replacement.endMinute) {
      next.push(entry);
      continue;
    }
    if (entry.startMinute < replacement.startMinute) {
      next.push({
        ...entry,
        id: `${entry.id}-before-${replacement.startMinute}`,
        endMinute: replacement.startMinute,
      });
    }
    if (entry.endMinute > replacement.endMinute) {
      next.push({
        ...entry,
        id: `${entry.id}-after-${replacement.endMinute}`,
        startMinute: replacement.endMinute,
      });
    }
  }
  next.push(replacement);
  return normalizeDayPlanEntries(next);
}

function buildSeededDayPlan({ role = {}, dateKey = "", longTermPlan = null, weeklyPlan = null } = {}) {
  const normalizedDateKey = normalizeDateKey(dateKey);
  const random = createSeededRandom(
    getPlanningSeed(DAY_PLAN_SEED_NAMESPACE, role?.name, normalizedDateKey),
  );
  const profile = getRolePlanningProfile(role, random);
  const goals = normalizePlanGoals(longTermPlan?.goals);
  const weeklyFocus = Array.isArray(weeklyPlan?.dailyFocus)
    ? weeklyPlan.dailyFocus.find((item) => item?.dateKey === normalizedDateKey)
    : null;
  const primary = goals.find((goal) => goal.id === weeklyFocus?.goalId) ||
    goals.find((goal) => goal.status === "active" && goal.priority === 1) || goals[0];
  const wellbeing = goals.find((goal) => goal.id === "goal-wellbeing") || goals[1] || primary;
  const connection = goals.find((goal) => goal.id === "goal-connection") || goals[2] || wellbeing || primary;
  const weekday = getWeekdayIndex(normalizedDateKey);
  const wakeMinute = seededInteger(random, 415, 470);
  const morningFocusStart = Math.max(525, wakeMinute + 100);
  const lunchStart = 12 * 60;
  const afternoonStart = 13 * 60;
  const afternoonEnd = seededInteger(random, 16 * 60 + 35, 17 * 60 + 15);
  const eveningGoal = weekday >= 5 ? connection : primary;
  const focusText = weeklyFocus?.title || (primary?.title
    ? `推进「${primary.title}」的一个阶段`
    : "完成今天最重要的主要事项");
  const afternoonText = profile.isStudent
    ? "整理笔记、练习并收尾今天的学习进度"
    : profile.isCreative
      ? "整理灵感、打磨细节并收尾今天的创作进度"
      : "整理进度、处理收尾事项并完成今天的主要工作";
  const eveningText = eveningGoal?.title
    ? `用轻松的节奏照顾「${eveningGoal.title}」`
    : "整理个人生活并留出休息时间";
  let entries = normalizeDayPlanEntries([
    { id: "night-sleep", startMinute: 0, endMinute: wakeMinute, kind: "sleep", activity: "夜间睡眠", location: profile.home, environment: "卧室，安静昏暗", mood: "熟睡", priority: 1 },
    { id: "morning-routine", startMinute: wakeMinute, endMinute: wakeMinute + 35, kind: "routine", activity: "起床、洗漱并整理自己", location: profile.home, environment: "卧室与洗手间", mood: "慢慢清醒", priority: 2 },
    { id: "breakfast", startMinute: wakeMinute + 35, endMinute: wakeMinute + 70, kind: "meal", activity: "吃早餐并确认今天的重点", location: profile.home, environment: "餐桌或厨房", mood: "平静", priority: 2 },
    { id: "morning-focus", startMinute: wakeMinute + 70, endMinute: lunchStart, kind: profile.mainKind, activity: focusText, location: profile.mainLocation, environment: profile.mainEnvironment, mood: "专注", priority: 1, ...(primary?.id ? { goalId: primary.id } : {}) },
    { id: "lunch", startMinute: lunchStart, endMinute: afternoonStart, kind: "meal", activity: "吃午饭并让自己休息一会儿", location: profile.mainLocation, environment: profile.mainLocation === profile.home ? "家里的餐桌" : "附近餐区或餐厅", mood: "放松", priority: 2 },
    { id: "afternoon-focus", startMinute: afternoonStart, endMinute: afternoonEnd, kind: profile.mainKind, activity: afternoonText, location: profile.mainLocation, environment: profile.mainEnvironment, mood: "稳定推进", priority: 1, ...(primary?.id ? { goalId: primary.id } : {}) },
    { id: "afternoon-wrap", startMinute: afternoonEnd, endMinute: 18 * 60, kind: "routine", activity: "收尾今天的进度，整理随身物品并缓一缓", location: profile.mainLocation, environment: profile.mainEnvironment, mood: "渐渐收尾", priority: 2, ...(primary?.id ? { goalId: primary.id } : {}) },
    { id: "movement", startMinute: 18 * 60, endMinute: 19 * 60, kind: "exercise", activity: "散步或做一点轻运动，恢复精神", location: "附近公园", environment: "树荫下的小路或安静的活动区", mood: "舒展开来", priority: 2, ...(wellbeing?.id ? { goalId: wellbeing.id } : {}) },
    { id: "dinner", startMinute: 19 * 60, endMinute: 19 * 60 + 45, kind: "meal", activity: "准备并吃晚饭", location: profile.home, environment: "家里的餐桌", mood: "安稳", priority: 2 },
    { id: "evening-plan", startMinute: 19 * 60 + 45, endMinute: 22 * 60, kind: weekday >= 5 ? "leisure" : "routine", activity: eveningText, location: profile.home, environment: "客厅、书桌或喜欢的角落", mood: "放松", priority: 2, ...(eveningGoal?.id ? { goalId: eveningGoal.id } : {}) },
    { id: "night-prep", startMinute: 22 * 60, endMinute: 22 * 60 + 30, kind: "routine", activity: "为明天做简单准备并放慢节奏", location: profile.home, environment: "卧室或书桌旁", mood: "安静", priority: 2 },
    { id: "sleep", startMinute: 22 * 60 + 30, endMinute: 1440, kind: "sleep", activity: "准备入睡并进入夜间睡眠", location: profile.home, environment: "卧室，灯光很暗", mood: "困倦", priority: 1 },
  ]);

  const commitments = normalizePlanCommitments(weeklyPlan?.commitments, {
    weekKey: getWeekStartDateKey(normalizedDateKey),
    requireTime: true,
  }).filter((commitment) => commitment.dateKey === normalizedDateKey);
  for (const commitment of commitments) {
    entries = overlayDayPlanEntry(entries, {
      id: makePlanningId("day", commitment.id, `${normalizedDateKey}:${commitment.id}`),
      ...
      ...(commitment.goalId ? { goalId: commitment.goalId } : {}),
  ...
  return {
    ...
    summary: primary?.title
      ? `今天以「${primary.title}」为主线，并保留生活恢复和晚间准备。`
      : "今天以一项主要事项、生活恢复和晚间准备为主线。",
    focusGoalIds: [primary?.id, wellbeing?.id, eveningGoal?.id].filter(Boolean),
    entries,
  };
}

function hasUsableDayPlan(entries) {
  return Array.isArray(entries) &&
    entries.length >= 6 &&
    entries[0]?.startMinute === 0 &&
    entries.at(-1)?.endMinute === 1440 &&
    entries.some((entry) => SLEEP_KINDS.has(entry.kind)) &&
    entries.every((entry, index) => (
      Boolean(normalizeLocation(entry.location)) &&
      entry.endMinute > entry.startMinute &&
      (index === 0 || entry.startMinute === entries[index - 1].endMinute)
    ));
}

function buildScheduleFromDayPlan({ role = {}, dateKey = "", seed, dayPlan = null } = {}) {
  const outlineEntries = normalizeDayPlanEntries(dayPlan?.entries);
  if (!hasUsableDayPlan(outlineEntries)) {
    return [];
  }
  ...
  return hasUsableDailySchedule(normalized)
    ? normalized
    : [];
}

function attachDayPlanMetadata(entries, dayPlan) {
  const outlineEntries = normalizeDayPlanEntries(dayPlan?.entries);
  if (!Array.isArray(entries) || outlineEntries.length === 0) {
    return Array.isArray(entries) ? entries : [];
  }
  return entries.map((entry) => {
    if (isTransitionEntry(entry) || entry.goalId || entry.commitmentId) {
      return entry;
    }
    let best = null;
    let bestOverlap = 0;
    for (const outlineEntry of outlineEntries) {
      const overlap = Math.max(0, Math.min(entry.endMinute, outlineEntry.endMinute) - Math.max(entry.startMinute, outlineEntry.startMinute));
    ...
    return {
      ...entry,
      ...(best.goalId ? { goalId: best.goalId } : {}),
      ...(best.commitmentId ? { commitmentId: best.commitmentId } : {}),
    };
  });
}

function normalizeLongTermPlanPayload(value) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const goals = normalizePlanGoals(source.goals);
  if (goals.length === 0) {
    return null;
  }
  return {
    summary: normalizeText(source.summary, "围绕长期目标、生活恢复和关系联结推进。", 360),
    goals,
    routines: normalizePlanningTextList(source.routines, 8, 180),
    constraints: normalizePlanningTextList(source.constraints, 8, 220),
  };
}

function normalizeWeeklyPlanPayload(value, weekKey) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const normalizedWeekKey = getWeekStartDateKey(source.weekKey || weekKey);
  if (!normalizedWeekKey || normalizedWeekKey !== getWeekStartDateKey(weekKey)) {
    return null;
  }
  const weekDates = new Set(getWeekDateKeys(normalizedWeekKey));
  const dailyFocus = (Array.isArray(source.dailyFocus) ? source.dailyFocus : [])
    .map((raw) => {
      const entry = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
      const dateKey = normalizeDateKey(entry.dateKey ?? entry.date);
      const title = normalizeText(entry.title ?? entry.activity ?? entry.focus, "", 180);
      const goalId = normalizePlanningReference(entry.goalId ?? entry.goal_id, 96);
      if (!dateKey || !weekDates.has(dateKey) || !title) {
        return null;
      }
      return {
        dateKey,
        title,
        ...(goalId ? { goalId } : {}),
        ...(normalizeText(entry.mode, "", 32) ? { mode: normalizeText(entry.mode, "", 32) } : {}),
      };
    })
    .filter(Boolean)
  ...
  const focusGoalIds = [...new Set((Array.isArray(source.focusGoalIds) ? source.focusGoalIds : [])
    .map((item) => normalizePlanningReference(item, 96))
    .filter(Boolean))]
    .slice(0, 8);
  return {
    weekKey: normalizedWeekKey,
    summary: normalizeText(source.summary, "本周围绕主线目标和稳定生活节奏安排。", 360),
    focusGoalIds,
    dailyFocus,
    commitments: normalizePlanCommitments(source.commitments, {
      weekKey: normalizedWeekKey,
      requireTime: true,
    }),
    outcomes: (Array.isArray(source.outcomes) ? source.outcomes : [])
      .map((raw) => {
        ...
        const activity = normalizeText(item.activity, "", 180);
        const status = String(item.status || "").trim().toLocaleLowerCase();
        if (!dateKey || !weekDates.has(dateKey) || !activity || ![
          "completed",
          "skipped",
          "failed",
        ].includes(status)) {
        ...
        const goalId = normalizePlanningReference(item.goalId, 96);
        return {
          id: normalizePlanningReference(item.id ?? item.outcomeId, 120) ||
            makePlanningId("outcome", "", `${dateKey}:${activity}:${status}`),
          dateKey,
          activity,
          status,
          ...(goalId ? { goalId } : {}),
          ...(normalizeText(item.recordedAt, "", 80) ? { recordedAt: normalizeText(item.recordedAt, "", 80) } : {}),
        };
      })
      .filter(Boolean)
      .slice(-24),
...
function normalizeDayPlanPayload(value, dateKey) {
  const source = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const normalizedDateKey = normalizeDateKey(source.dateKey || dateKey);
  if (!normalizedDateKey || normalizedDateKey !== normalizeDateKey(dateKey)) {
    return null;
  }
  const entries = normalizeDayPlanEntries(source.entries);
  if (!hasUsableDayPlan(entries)) {
    return null;
  }
  return {
    dateKey: normalizedDateKey,
    summary: normalizeText(source.summary, "今天以主线任务、生活恢复和晚间准备为主。", 360),
    focusGoalIds: [...new Set((Array.isArray(source.focusGoalIds) ? source.focusGoalIds : [])
      .map((item) => normalizePlanningReference(item, 96))
      .filter(Boolean))]
      .slice(0, 8),
    entries,
...
function buildSeededSchedule({ role = {}, dateKey = "", seed, dayPlan = null } = {}) {
  const plannedSchedule = buildScheduleFromDayPlan({
    role,
    dateKey,
    seed,
    dayPlan,
...
function createRoleScheduleManager({
  ...
  async function withPlanningLock(lockKey, work) {
    if (planningLocks.has(lockKey)) {
      return planningLocks.get(lockKey);
    }
    const promise = Promise.resolve().then(work);
    planningLocks.set(lockKey, promise);
    try {
      return await promise;
    } finally {
      planningLocks.delete(lockKey);
    }
  }

  async function persistPlanningRecord(existing, record) {
  ...
  async function ensureLongTermPlan(roleOrName) {
    ...
    return withPlanningLock(`long:${roleNameKey}`, async () => {
      const existing = await db.findOneAsync({
        type: LONG_TERM_PLAN_RECORD_TYPE,
        roleNameKey,
      });
      const payload = Number(existing?.planningVersion) === PLANNING_VERSION
        ? normalizeLongTermPlanPayload(existing)
        : null;
      if (payload) {
        return {
          ...existing,
          ...
          roleNameKey,
          timezone: existing.timezone || resolvedTimezone,
        };
      }
      ...
      const record = {
        ...
        longTermSeed: getPlanningSeed(LONG_TERM_PLAN_SEED_NAMESPACE, roleName),
        source: "seeded",
        ...buildSeededLongTermPlan({ role: role || { name: roleName } }),
        createdAt: existing?.createdAt || timestamp,
        updatedAt: timestamp,
      };
      return persistPlanningRecord(existing, record);
    });
  }

  async function ensureWeeklyPlan(roleOrName, dateKey, { longTermPlan = null } = {}) {
    const role = typeof roleOrName === "string" ? await resolveRole(roleOrName) : roleOrName;
    const roleName = role?.name || (typeof roleOrName === "string" ? roleOrName : "");
    const roleNameKey = normalizeRoleNameKey(roleName);
    const weekKey = getWeekStartDateKey(dateKey);
    if (!roleNameKey || !weekKey) {
      return null;
    }
    return withPlanningLock(`week:${roleNameKey}:${weekKey}`, async () => {
      const existing = await db.findOneAsync({
        type: WEEKLY_PLAN_RECORD_TYPE,
        roleNameKey,
        weekKey,
      });
      const payload = Number(existing?.planningVersion) === PLANNING_VERSION
        ? normalizeWeeklyPlanPayload(existing, weekKey)
        : null;
      if (payload) {
        return {
          ...existing,
      ...
      const longPlan = longTermPlan || await ensureLongTermPlan(role || roleName);
      const timestamp = new Date().toISOString();
      const record = {
        type: WEEKLY_PLAN_RECORD_TYPE,
        planningVersion: PLANNING_VERSION,
        ...
        weekKey,
        timezone: resolvedTimezone,
        weeklySeed: getPlanningSeed(WEEKLY_PLAN_SEED_NAMESPACE, roleName, weekKey),
        source: "seeded",
        ...buildSeededWeeklyPlan({
          role: role || { name: roleName },
          weekKey,
          longTermPlan: longPlan,
        }),
        createdAt: existing?.createdAt || timestamp,
        updatedAt: timestamp,
      };
  ...
  async function ensureDayPlan(
    roleOrName,
    dateKey,
    { longTermPlan = null, weeklyPlan = null } = {},
  ) {
    const role = typeof roleOrName === "string" ? await resolveRole(roleOrName) : roleOrName;
    const roleName = role?.name || (typeof roleOrName === "string" ? roleOrName : "");
    const roleNameKey = normalizeRoleNameKey(roleName);
    const normalizedDateKey = normalizeDateKey(dateKey);
    if (!roleNameKey || !normalizedDateKey) {
      return null;
    }
    return withPlanningLock(`day:${roleNameKey}:${normalizedDateKey}`, async () => {
      const existing = await db.findOneAsync({
      ...
      const payload = Number(existing?.planningVersion) === PLANNING_VERSION
        ? normalizeDayPlanPayload(existing, normalizedDateKey)
        : null;
      if (payload) {
        return {
          ...existing,
      ...
      const longPlan = longTermPlan || await ensureLongTermPlan(role || roleName);
      const weekPlan = weeklyPlan || await ensureWeeklyPlan(role || roleName, normalizedDateKey, {
        longTermPlan: longPlan,
      });
      const timestamp = new Date().toISOString();
      const record = {
        ...
        weekKey: getWeekStartDateKey(normalizedDateKey),
        timezone: resolvedTimezone,
        dayPlanSeed: getPlanningSeed(DAY_PLAN_SEED_NAMESPACE, roleName, normalizedDateKey),
        source: "seeded",
        ...buildSeededDayPlan({
          role: role || { name: roleName },
          dateKey: normalizedDateKey,
  ...
  async function ensurePlanningContext(roleOrName, dateKey, { includeTomorrow = false } = {}) {
    const role = typeof roleOrName === "string" ? await resolveRole(roleOrName) : roleOrName;
    const roleName = role?.name || (typeof roleOrName === "string" ? roleOrName : "");
    const normalizedDateKey = normalizeDateKey(dateKey);
    if (!roleName || !normalizedDateKey) {
      return null;
    }
    const longTermPlan = await ensureLongTermPlan(role || roleName);
    const weeklyPlan = await ensureWeeklyPlan(role || roleName, normalizedDateKey, { longTermPlan });
    const dayPlan = await ensureDayPlan(role || roleName, normalizedDateKey, {
      longTermPlan,
      weeklyPlan,
    });
    let tomorrowPlan = null;
    if (includeTomorrow) {
      const tomorrowDateKey = shiftDateKey(normalizedDateKey, 1);
      const tomorrowWeekPlan = await ensureWeeklyPlan(role || roleName, tomorrowDateKey, {
        longTermPlan,
      });
      tomorrowPlan = await ensureDayPlan(role || roleName, tomorrowDateKey, {
        longTermPlan,
        weeklyPlan: tomorrowWeekPlan,
      });
    }
    return {
      roleName,
      dateKey: normalizedDateKey,
      weekKey: weeklyPlan?.weekKey || getWeekStartDateKey(normalizedDateKey),
      longTermPlan,
      weeklyPlan,
      dayPlan,
  ...
  function formatPlanEntries(entries) {
    return (Array.isArray(entries) ? entries : [])
      .map((entry) => `${formatMinute(entry.startMinute)}-${formatMinute(entry.endMinute)} ${entry.activity}${entry.location ? ` [${entry.location}]` : ""}`)
      .join("\n");
  }

  function formatPlanningOverview(overview, horizon = "all") {
    if (!overview?.longTermPlan || !overview?.weeklyPlan || !overview?.dayPlan) {
      return "当前还没有可用的分层计划。";
    }
    const requested = String(horizon || "all").trim().toLocaleLowerCase();
    const showLong = ["all", "long", "longterm", "长期", "长期计划"].includes(requested);
    const showWeek = ["all", "week", "weekly", "本周", "周"].includes(requested);
    const showToday = ["all", "today", "今天", "day"].includes(requested);
    const showTomorrow = ["all", "tomorrow", "明天", "次日"].includes(requested);
    const sections = [];
    if (showLong) {
      const goals = overview.longTermPlan.goals
        .map((goal) => `- ${goal.title}（${goal.area}，优先级 ${goal.priority}，进度 ${goal.progress}%）`)
        .join("\n");
      sections.push(`长期主线\n${overview.longTermPlan.summary}\n${goals}`);
    }
    if (showWeek) {
      const focus = overview.weeklyPlan.dailyFocus
        .map((entry) => `- ${entry.dateKey}：${entry.title}`)
        .join("\n");
      const commitments = overview.weeklyPlan.commitments.length > 0
        ? `\n固定安排：\n${overview.weeklyPlan.commitments.map((entry) => `- ${entry.dateKey} ${formatMinute(entry.startMinute)}-${formatMinute(entry.endMinute)} ${entry.activity}`).join("\n")}`
        : "";
      const outcomes = Array.isArray(overview.weeklyPlan.outcomes) && overview.weeklyPlan.outcomes.length > 0
        ? `\n近期进展：\n${overview.weeklyPlan.outcomes.slice(-5).map((entry) => `- ${entry.dateKey} ${entry.activity}：${entry.status === "completed" ? "已完成" : entry.status === "skipped" ? "暂时跳过" : "未完成"}`).join("\n")}`
        : "";
      sections.push(`本周计划（${overview.weekKey} 起）\n${overview.weeklyPlan.summary}\n${focus}${commitments}${outcomes}`);
    }
    if (showToday) {
      sections.push(`当天提纲（${overview.dayPlan.dateKey}）\n${overview.dayPlan.summary}\n${formatPlanEntries(overview.dayPlan.entries)}`);
    }
    if (showTomorrow && overview.tomorrowPlan) {
      sections.push(`次日提纲（${overview.tomorrowPlan.dateKey}）\n${overview.tomorrowPlan.summary}\n${formatPlanEntries(overview.tomorrowPlan.entries)}`);
    }
  ...
  function normalizeStoredSchedule(record) {
    ...
    return hasUsableDailySchedule(entries)
      ? {
          ...record,
  ...
  async function ensureDailySchedule(
    ...
    const promise = (async () => {
      ...
      let planning = null;
      try {
        planning = await ensurePlanningContext(role || roleName, dateKey);
      } catch (error) {
        // Planning enriches the daily schedule, but a transient planning-store
        // failure must never leave the character without a current reality.
        logger.warn?.(`生成「${roleName}」分层计划失败，继续使用日程兜底:`, error.message || error);
      }
      ...
      if (typeof generateSchedule === "function" && role) {
        try {
          ...
          entries = attachDayPlanMetadata(
            normalizeScheduleEntries(parseSchedulePayload(generated)),
            planning?.dayPlan,
          );
          if (hasUsableDailySchedule(entries)) {
            source = "model";
          } else {
            generationError = "模型日程不完整、存在过多分钟碎片，已改用确定性兜底日程";
            entries = [];
          }
        } catch (error) {
      ...
      if (entries.length === 0) {
        entries = buildSeededSchedule({
          role,
          dateKey,
          seed: dailySeed,
          dayPlan: planning?.dayPlan,
        });
        source = "seeded";
      }
      ...
      const record = {
        ...
        entries,
        source,
        ...(planning?.longTermPlan ? {
          planningVersion: PLANNING_VERSION,
          longTermPlanId: planning.longTermPlan._id || "",
          longTermPlanUpdatedAt: planning.longTermPlan.updatedAt || "",
          weekKey: planning.weeklyPlan?.weekKey || "",
          weeklyPlanId: planning.weeklyPlan?._id || "",
          weeklyPlanUpdatedAt: planning.weeklyPlan?.updatedAt || "",
          dayPlanId: planning.dayPlan?._id || "",
          dayPlanUpdatedAt: planning.dayPlan?.updatedAt || "",
        } : {}),
        createdAt: timestamp,
        updatedAt: timestamp,
  ...
  async function recordPlanningOutcome(role, outcome, at = now()) {
    if (!outcome || outcome.planningFeedbackAppliedAt || ![
      "completed",
      "skipped",
      "failed",
    ...
    const dateKey = normalizeDateKey(outcome.dateKey);
    if (!dateKey || !role?.name) {
      return outcome;
    }
    try {
      const timestamp = getBehaviorTimestamp(at);
      const goalId = normalizePlanningReference(outcome.goalId, 96);
      const longTermPlan = await ensureLongTermPlan(role);
      let effectiveLongTermPlan = longTermPlan;
      if (goalId && longTermPlan?.goals?.some((goal) => goal.id === goalId)) {
        const progressStep = outcome.status === "completed"
          ? outcome.commitmentId ? 2 : 1
          : 0;
        const goals = longTermPlan.goals.map((goal) => (
          goal.id === goalId
            ? {
                ...goal,
                progress: Math.min(99, Math.max(0, Number(goal.progress) || 0) + progressStep),
                lastOutcomeAt: timestamp,
                lastOutcomeStatus: outcome.status,
              }
            : goal
        ));
        effectiveLongTermPlan = await persistPlanningRecord(longTermPlan, {
          ...longTermPlan,
          goals,
          updatedAt: timestamp,
        });
      ...
      const weeklyPlan = await ensureWeeklyPlan(role, dateKey, {
        longTermPlan: effectiveLongTermPlan,
      });
      if (weeklyPlan) {
        const outcomeId = normalizePlanningReference(
          outcome._id,
          120,
        ) || makePlanningId(
          "outcome",
          "",
          `${dateKey}:${outcome.entryStartMinute}:${outcome.activity}:${outcome.status}`,
        );
        const priorOutcomes = Array.isArray(weeklyPlan.outcomes) ? weeklyPlan.outcomes : [];
        const outcomes = priorOutcomes.some((item) => item.id === outcomeId)
          ? priorOutcomes
          : [...priorOutcomes, {
              id: outcomeId,
              dateKey,
              activity: normalizeText(outcome.activity, "当天安排", 180),
              status: outcome.status,
              ...(goalId ? { goalId } : {}),
              recordedAt: timestamp,
            }].slice(-24);
        await persistPlanningRecord(weeklyPlan, {
          ...weeklyPlan,
          outcomes,
          updatedAt: timestamp,
        });
      ...
      if (outcome._id) {
        await db.updateAsync(
          { _id: outcome._id },
          { $set: { planningFeedbackAppliedAt: timestamp, planningGoalId: goalId } },
        );
      }
      return {
        ...outcome,
        planningFeedbackAppliedAt: timestamp,
        planningGoalId: goalId,
      };
    } catch (error) {
      logger.warn?.("写入角色计划进展失败，保留原行为结果:", error.message || error);
      return outcome;
    }
  }
  ...
  async function runBehaviorAttempt({ role, entry, state, record = null, at = now(), isRetry = false }) {
    ...
    const baseRecord = record || {
      ...
      kind: entry.kind,
      environment: entry.environment,
      ...(normalizePlanningReference(entry.goalId, 96) ? {
        goalId: normalizePlanningReference(entry.goalId, 96),
      } : {}),
      ...(normalizePlanningReference(entry.commitmentId, 96) ? {
        commitmentId: normalizePlanningReference(entry.commitmentId, 96),
      } : {}),
      attempts: [],
      createdAt: timestamp,
    };
    const attempts = [...(baseRecord.attempts || []), attempt];
    const finalize = async (nextRecord) => recordPlanningOutcome(
      role,
      await saveBehaviorOutcome(nextRecord),
      at,
    );
  ...
  async function getState(roleName, { scope = null, at = now() } = {}) {
    ...
    let planning = null;
    try {
      planning = await ensurePlanningContext(
        schedule.roleName || roleName,
        schedule.dateKey,
        { includeTomorrow: true },
      );
    } catch (error) {
      logger.warn?.("读取角色分层计划失败，继续使用日程状态:", error.message || error);
    }
  ...
  function buildPlanningRuntimeContext(planning, currentEntry = null) {
    if (!planning?.longTermPlan || !planning?.weeklyPlan || !planning?.dayPlan) {
      return "";
    }
    const goals = Array.isArray(planning.longTermPlan.goals)
      ? planning.longTermPlan.goals
      : [];
    const goalById = new Map(goals.map((goal) => [goal.id, goal]));
    const currentGoal = currentEntry?.goalId ? goalById.get(currentEntry.goalId) : null;
    const todayFocus = Array.isArray(planning.weeklyPlan.dailyFocus)
      ? planning.weeklyPlan.dailyFocus.find((item) => item.dateKey === planning.dayPlan.dateKey)
      : null;
    const weeklyGoals = (planning.weeklyPlan.focusGoalIds || [])
      .map((goalId) => goalById.get(goalId)?.title)
      .filter(Boolean)
      .slice(0, 3);
    const tomorrowFocus = Array.isArray(planning.tomorrowPlan?.entries)
      ? planning.tomorrowPlan.entries.find((entry) => (
        !SLEEP_KINDS.has(entry.kind) &&
        !["meal", "rest", "routine"].includes(entry.kind) &&
        entry.priority <= 2
      ))
      : null;
    return [
      "角色分层生活计划（这是内部的长期现实，不要主动解释为后台系统）：",
      `长期主线：${goals.filter((goal) => goal.status === "active").slice(0, 3).map((goal) => goal.title).join("；") || planning.longTermPlan.summary}`,
      `本周重点：${weeklyGoals.join("；") || planning.weeklyPlan.summary}`,
      `当天提纲：${todayFocus?.title || planning.dayPlan.summary}`,
      tomorrowFocus ? `明天预定重点：${tomorrowFocus.activity}。` : "",
      currentGoal ? `当前活动关联目标：${currentGoal.title}。` : "",
      "当前分钟级日程和已发生的实体状态优先；如果临时变化，只自然地调整还没发生的安排，不要改写已经发生的事情。",
    ].filter(Boolean).join("\n");
  }
  ...
  function buildRuntimeContextFromState(state) {
    ...
    const planningContext = buildPlanningRuntimeContext(state.planning, entry);
    return [
  ...
  async function getTodaySchedule(roleName, at = now()) {
    ...
    const planning = await ensurePlanningContext(
      schedule.roleName || roleName,
      schedule.dateKey,
      { includeTomorrow: true },
    );
    return { ...schedule, planning, formatted: formatSchedule(schedule) };
  }

  async function getPlanningOverview(roleName, at = now()) {
    const dateKey = getDateKey(at, resolvedTimezone);
    return ensurePlanningContext(roleName, dateKey, { includeTomorrow: true });
  }
  ...
  async function resetRoleStateAndSchedule(
    ...
    const cleared = {
      ...
      longTermPlans: await removeRoleRecords(LONG_TERM_PLAN_RECORD_TYPE),
      weeklyPlans: await removeRoleRecords(WEEKLY_PLAN_RECORD_TYPE),
      dayPlans: await removeRoleRecords(DAY_PLAN_RECORD_TYPE),
    };
  ...
  async function tick(at = now()) {
    ...
    try {
      ...
      for (const role of roles) {
        ...
        await ensurePlanningContext(role, getDateKey(at, resolvedTimezone), {
          includeTomorrow: true,
        });
...
module.exports = {
  ...
  buildSeededSchedule,
  buildSeededLongTermPlan,
  buildSeededWeeklyPlan,
  buildSeededDayPlan,
  buildScheduleFromDayPlan,
  buildFallbackSchedule,
  createSeededRandom,
