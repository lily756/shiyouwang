...
function buildMiniMaxH3VideoPrompt(
  rawPrompt,
  { mode = "freeform", allowOnScreenText = false } = {},
) {
  const prompt = normalizePrompt(rawPrompt);
  if (!prompt) {
    return "";
  ...
  // H3 receives reference media as typed content items. The @图片/@视频
  // tokens are an internal Seedance convention, so turn them into natural
  // language before sending the text item to H3.
  const normalizedPrompt = prompt
    .replace(/参考\s*@图片(\d+)/g, "参考图$1")
    .replace(/参考\s*@视频(\d+)/g, "参考视频$1")
  ...
  const constraints = [
    "主体身份和关键外观保持稳定，动作连贯自然。",
    "镜头和动作按先后顺序执行，避免在同一短片中堆叠互相冲突的运动。",
  ];
  if (!allowOnScreenText) {
    constraints.push("未明确要求时不要生成字幕、Logo 或水印。" );
  }
  return `${normalizedPrompt}\n\nH3 执行约束：${constraints.join("")}`;
}

function getMediaPromptSystemInstruction(mode = "freeform") {
  if (normalizeMediaPromptMode(mode) === "freeform") {
    return [
      "媒体生成提示词模式：freeform。generate_character_image、edit_reference_image 和 generate_character_video 收到的 prompt/instruction 是模型生成的原始媒体意图；图片任务若开启后台提示词优化，会再结合角色 system prompt 与最近对话整理最终提示词。未开启优化时才会原样交给 provider。",
      "媒体 Function Call 必须同时生成 reply（发给用户的即时对话）和 prompt/instruction（媒体意图）；caption 只用于成品发送时的配文。不要把 reply 或 caption 混入 prompt。",
