function createAdminFlow({
  ...
  function normalizeAction(text) {
    ...
    if (["设定图", "人设图", "角色图", "参考图", "reference", "portrait"].includes(value)) return "reference";
    if (["功能", "工具", "设置", "tools", "settings"].includes(value)) return "tools";
    return null;
  }

  function toolMenu(settings) {
    return `${formatToolStatus(settings)}\n\n请输入：时间、图片、图片编辑、视频、看图、搜索、生活、语音、文件、3D、工作区、Python、provider、状态 或 返回。`;
  }
  ...
  function normalizeTool(text) {
    ...
    if (["语音", "语音消息", "音频", "audio", "voice"].includes(value)) return { settingName: "audioEnabled", label: "语音消息" };
    if (["文件", "文件上传", "minimax文件", "file", "files"].includes(value)) return { settingName: "fileUploadEnabled", label: "MiniMax 文件上传" };
    if (["3d", "3d模型", "模型", "骨骼", "动画", "three.js", "threejs"].includes(value)) return { settingName: "threeDEnabled", label: "3D 模型与骨骼动画" };
    if (["工作区", "文件工作区", "workspace", "git"].includes(value)) return { settingName: "workspaceEnabled", label: "受控工作区与 Git" };
    if (["python", "python沙箱", "代码", "代码执行", "沙箱", "sandbox"].includes(value)) return { settingName: "codeExecutionEnabled", label: "Python 沙箱" };
    if (["provider", "服务商", "供应商", "媒体 provider", "媒体服务"].includes(value)) return { settingName: "provider", label: "媒体 provider" };
    return null;
  }
  ...
  async function handle(ctx, scope, flow, text) {
    ...
    if (flow.step === "choose-action") {
      ...
      if (action === "edit" || action === "delete" || action === "reference") {
        const roles = await getRoles();
        if (roles.length === 0) {
          const actionLabel = action === "edit" ? "编辑" : action === "delete" ? "删除" : "设置设定图的";
          await ctx.reply(`当前没有可${actionLabel}角色。\n\n${menu()}`);
          return;
        }
        const step = action === "edit" ? "edit-select" : action === "delete" ? "delete-select" : "reference-select";
        const actionLabel = action === "edit" ? "编辑" : action === "delete" ? "删除" : "设置人设图";
        await set(scope, step);
        await replyWithText(ctx, `请输入要${actionLabel}的角色名称：\n\n${formatRoleList(roles)}`);
        return;
      }
    ...
    if (flow.step === "tool-menu") {
      ...
      if (!tool) {
        await ctx.reply("未识别该工具。请输入：时间、图片、图片编辑、视频、看图、搜索、生活、语音、文件、3D、工作区、Python、状态 或 返回。");
        return;
      }
      if (tool.settingName === "provider") {
        await set(scope, "provider-select");
        await ctx.reply("请输入 provider 设置，例如：image=minimax 或 video=seedance。图片可选 seedream/newapi/minimax，视频可选 seedance/minimax。可连续设置两项。");
        return;
      }
    ...
    if (flow.step === "provider-select") {
      if (typeof setMediaProvider !== "function") {
        await ctx.reply("当前版本没有启用媒体 provider 管理。请重启机器人加载最新配置。" );
        return;
      }
      const assignments = value.split(/[\s,，;；]+/).map((item) => item.trim()).filter(Boolean);
      if (assignments.length === 0) {
        await ctx.reply("格式示例：image=minimax 或 video=seedance。请重新输入。");
        return;
      }
      const results = [];
      for (const assignment of assignments) {
        const match = assignment.match(/^(image|图片|video|视频)\s*[:=：]\s*([a-z0-9_-]+)$/i);
        if (!match) {
          results.push(`无法识别「${assignment}」`);
          continue;
        }
        const kind = ["image", "图片"].includes(match[1].toLowerCase()) ? "image" : "video";
        const result = await setMediaProvider(kind, match[2], scope.userId);
        results.push(result.ok ? `${kind === "image" ? "图片" : "视频"} provider 已切换为 ${match[2].toLowerCase()}` : result.error);
      }
      await set(scope, "tool-menu");
      await ctx.reply(`${results.join("\n")}\n\n${toolMenu(await getToolSettings())}`);
      return;
    }
    ...
    if (flow.step === "reference-select") {
      const role = findRole(await getRoles(), value);
      if (!role?.id) {
        await ctx.reply("没有找到该角色，请重新输入角色名称。");
        return;
      }
      await set(scope, "reference-upload", { roleId: role.id, roleName: role.name });
      await ctx.reply(`请直接上传「${role.name}」的人设图。发送 取消 可退出管理模式。`);
      return;
    }

    if (flow.step === "reference-upload") {
      await ctx.reply("请直接上传一张图片作为人设图，或发送 取消 退出管理模式。");
      return;
    }
    ...
    if (flow.step === "edit-field") {
      ...
      if (field === "systemPrompt") {
        ...
        if (!target?.systemPrompt) {
          await clear(scope);
          await ctx.reply("这个角色已不存在或提示词为空，已退出管理模式。请重新发送 /admin。");
          return;
        }
        ...
        await replyWithText(
          ctx,
          `「${target.name}」当前的 system prompt：\n\n${target.systemPrompt}\n\n请直接发送新的 system prompt。`,
        );
        return;
      }

      await set(scope, "edit-value", { ...flow.draft, field });
      const label = field === "name" ? "新名称" : "新简介";
