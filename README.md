...
- 角色图片：支持 Seedream 5.0 Pro 的 T2I 与 I2I；图片开始生成或编辑前、以及成图发送时，都会由当前角色结合最近对话各写一段自然的台词，不再使用固定进度文案。图片生成和编辑会创建独立后台任务，因此可与文字会话、其他图片任务和视频任务并行；重启后待处理任务会恢复。管理员保存的人设图按角色独立保存：角色相关的 T2I 默认会直接带入该角色人设图，角色入景、换装等 I2I 会同时传入场景图与人设图，以锁定脸部与参考图原生风格；不预设 2D/动漫或写实，真人照片保持摄影风格，插画、动漫、3D 等也保持各自原生风格。只有用户明确要求纯风景、纯物品、纯食物或不要人物时才不带人设图。未配置 Seedream 时会改走已有的 NewAPI 图片路径。
