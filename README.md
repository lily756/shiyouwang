...
- 角色图片：支持 Seedream 5.0 Pro 的 T2I 与 I2I；图片开始生成或编辑前、以及成图发送时，都会由当前角色结合最近对话各写一段自然的台词，不再使用固定进度文案。图片生成和编辑会创建独立后台任务，因此可与文字会话、其他图片任务和视频任务并行；重启后待处理任务会恢复。角色换装、遇到漂亮场景或自然的自拍/打卡时，会主动拍一张；每条消息最多一张。画面包含角色本人时优先使用该角色已保存的人设图保持外观和 2D 画风，纯风景、物品或食物则不强行让角色入镜。未配置 Seedream 时会改走已有的 NewAPI 图片路径。
