...
发送 `/admin` 后，按机器人提示输入“新增”“编辑”“删除”“查看”或“功能”，即可逐步完成角色的增删改查或管理工具开关。新增或编辑角色时，机器人会依次询问名称、简介和 system prompt；发送 `/cancel` 或“取消”会退出管理流程。
...
- 角色图片：支持 Seedream 5.0 Pro 的 T2I 与 I2I；图片会附带由当前角色根据最近对话写出的俏皮配文。未配置 Seedream 时可继续使用原有 NewAPI 图片路径。
...
纯文字消息使用 `OPENAI_MODEL`，它必须支持 OpenAI Chat Completions 的 `tools` / `tool_calls` 协议；否则普通对话仍可用，但无法可靠执行这些函数。图片和 sticker 会自动改用 `OPENAI_VISION_MODEL`；未配置该项时才回退到 `OPENAI_MODEL`。
...
为保持角色的 2D 画风和外观一致，视频必须先有角色设定图：管理员先在私聊中用 `/newchat <角色名>` 进入对应角色，然后说“生成一张角色设定图并保存”，或上传图片后说“把这张保存为角色设定图”。机器人会把图片写入本地 `role-assets/`，并在数据库中记录当前版本；之后生成该角色的视频时，自动以这张本地图片转换成 `data:` URL，作为 Seedance 的 `reference_image` 角色参考图提交，而非 `first_frame`。因此设定图只约束人物身份、2D 画风与未明确要求改变的外观，不会强制成为视频第一帧。普通用户和普通场景图都不能覆盖这项全局角色资产。
