...
- 角色视频：支持 Seedance 2.0 Mini 文生视频；角色会把明确的视频创作需求转成 Function Call，并将提交与轮询都放进后台任务，待 MP4 完成后主动发送到当前 Telegram 对话。
...
视频可使用 0～9 张参考图：纯文生视频不传图片；角色视频会选择当前角色的设定图；本轮上传图片和当前角色会话中的历史图片也可按顺序组合。函数调用中的 `reference_ids` 使用 `role`（当前角色设定图）、`current`（本轮上传）或运行时列出的 `img_` 历史编号；其顺序对应 Seedance 的 `@图片1`、`@图片2`……。所有参考图都以 `reference_image` 提交，不会被当作 `first_frame`。管理员可直接在私聊中依次发送 `/admin`、`设定图`、角色名称，再上传图片；不需要先开启角色对话。也可以在 `/newchat <角色名>` 后说“生成一张角色设定图并保存”，或上传图片后说“把这张保存为角色设定图”。机器人会把图片写入本地 `role-assets/`，并在数据库中记录当前版本；设定图仅约束人物身份、原生风格与未明确要求改变的外观。普通用户和普通场景图都不能覆盖这项全局角色资产。
