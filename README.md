...
- 角色图片：支持 Seedream 5.0 Pro 的 T2I 与 I2I；图片开始生成或编辑前、以及成图发送时，都会由当前角色结合最近对话各写一段自然的台词，不再使用固定进度文案。图片生成和编辑会创建独立后台任务，因此可与文字会话、其他图片任务和视频任务并行；重启后待处理任务会恢复。`freeform` 模式下管理员保存的人设图只有在 Function Call 明确设置 `include_current_role=true` 时才会带入；`guided` 模式才保留旧的自动角色参考策略。未配置 Seedream 时会改走已有的 NewAPI 图片路径。
- 角色视频：支持 Seedance 2.0 Mini 文生视频，以及 0～9 张图片、0～3 段视频的参考生成；角色会把明确的视频创作需求转成 Function Call，并将提交与轮询都放进后台任务，待 MP4 完成后主动发送到当前 Telegram 对话。
- 媒体提示词模式：默认 `MEDIA_PROMPT_MODE=freeform`。在该模式下，`generate_character_image`、`edit_reference_image` 和 `generate_character_video` 是薄 Function：模型生成的最终 `prompt`/`instruction` 会原样交给图片或视频 provider，程序不会自动添加角色、场景、镜头、画风、无文字或画质约束；角色设定图、历史图片和视频也只有在 Function Call 明确选择时才会作为参考。若希望恢复旧的服务端提示词扩展与风格约束，可设置 `MEDIA_PROMPT_MODE=guided` 后重启。两种模式都会保留后台队列、权限、参考素材编号校验、任务日志和失败重试。
- 媒体 Function 的 `reply` 和 `prompt` 会在同一次调用中生成：`reply` 立即发给用户，`prompt` 原样提交给图片/视频模型，图片完成后的 `caption` 另行发送。带 `reply` 的媒体调用不会再触发第二次模型总结，避免重复回复；旧模型缺少 `reply` 时仍兼容原来的二次回复流程。freeform Skill 会把“自拍”“前置摄像头”“随手拍”等自然语言转换成手机摄影语言，并要求只有明确选择 `role` 时才使用角色设定图；图片还支持 `aspect_ratio`（例如自拍常用 `9:16`）。
...
视频提示词在 `freeform` 模式下由模型自由组织并原样提交；切换到 `guided` 后才会按 Seedance 2.0 工程化规范补充主体、动作、场景、镜头以及高清、稳定、无水印等约束。

视频可使用 0～9 张图片和 0～3 段视频参考：纯文生视频两个列表都不传素材；在 `freeform` 模式下只有 Function Call 明确选择 `role` 时才会带当前角色设定图，`guided` 模式保留旧的角色参考策略。本轮上传图片和当前角色会话中的历史图片也可按顺序组合。函数调用中的 `reference_ids` 使用 `role`（当前角色设定图）、`current`（本轮上传）或运行时列出的 `img_` 历史编号；其顺序对应 Seedance 的 `@图片1`、`@图片2`……。所有图片都以 `reference_image` 提交，不会被当作 `first_frame`。Telegram 发来的视频会本地保存为 `vid_` 历史编号；只有用户明确说“参考刚才的视频动作/节奏/运镜生成……”时，Function Call 才会将最多 3 个 `video_reference_ids` 按顺序映射为 `@视频1`、`@视频2`……并以 `reference_video` 提交，普通视频生成不会擅自使用它们。由于上游 JSON `data:` URL 限制，单段 Telegram 视频参考当前约限 3MB；大视频需要先压缩或接入对象存储。管理员可直接在私聊中依次发送 `/admin`、`设定图`、角色名称，再上传图片；不需要先开启角色对话。也可以在 `/newchat <角色名>` 后说“生成一张角色设定图并保存”，或上传图片后说“把这张保存为角色设定图”。机器人会把图片写入本地 `role-assets/`，并在数据库中记录当前版本；设定图仅约束人物身份、原生风格与未明确要求改变的外观。普通用户和普通场景图都不能覆盖这项全局角色资产。
